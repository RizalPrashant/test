		...
		try 
		{
			...
			for(int i=0;i<ROWS; i++){
				...
				for(int j = 0; j< COLS;j++)
				{
					...
					{
						scan2.close();
						throw new InvalidFileFormatException("Invalid format - only uses chars O, T, 1, 2, X");
					}
			...
			if (startingPoint == null){					// no starting point
				throw new InvalidFileFormatException("No start point");
			}
			if (endingPoint == null){			//no ending point
